...
export const driverAuthRoutes: FastifyPluginAsync = async (server: FastifyInstance) => {
  server.post('/register', async (request, reply) => {
    const { name, cpf, email, phone, pixKey, pixType, companyUuid } = request.body as any;

    try {
      // 1. Validação Real da Chave PIX (Anti-fraude e Anti-erros)
      await AsaasClient.validatePixKey(pixKey, pixType);
      ...
      await connection.query(
        `INSERT INTO drivers (uuid, company_uuid, name, phone, email, status, meta, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, 'pending', ?, NOW(), NOW())`,
        [driverUuid, companyUuid, name, phone, email, metaJSON]
      );
      connection.release();

      logger.info(`Driver ${name} registered successfully. Status: PENDING.`);
