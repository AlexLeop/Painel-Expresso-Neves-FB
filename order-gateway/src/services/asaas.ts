...
const baseURL = config.ASAAS_API_KEY?.startsWith('$aact_') 
...
export class AsaasClient {
   ...
   */
  static async createCustomer(name: string, cpfCnpj: string, email?: string, phone?: string) {
    try {
      ...
      logger.error('Asaas createCustomer error', error.response?.data || error.message);
      throw error;
    }
  }
   ...
   */
  static async createPixCharge(customerId: string, value: number, description: string, dueDate: string) {
    try {
      ...
      logger.error('Asaas createPixCharge error', error.response?.data || error.message);
      throw error;
    }
  }
   ...
   */
  static async transferPix(value: number, pixAddressKey: string, pixAddressKeyType: string, description: string, idempotencyKey?: string) {
    try {
      const headers: any = {};
      if (idempotencyKey) {
      ...
      logger.error('Asaas transferPix error', error.response?.data || error.message);
      throw error;
    }
  }
   ...
   */
  static async validatePixKey(pixAddressKey: string, pixAddressKeyType: string): Promise<boolean> {
    try {
      ...
      if (pixAddressKey.length < 5) throw new Error('Chave PIX muito curta');
      return true;
    } catch (error: any) {
      logger.error('Asaas PIX Validation error', error.response?.data || error.message);
  ...
  static async getBalance() {
    try {
      ...
      logger.error('Asaas getBalance error', error.response?.data || error.message);
      throw error;
    }
  }
}
